...
class DNNMarketDirectionPredictor:
    ...
    def __init__(self, input_size: int):
        ...
        # use BCEWithLogitsLoss and return raw logits from the model for numerical stability
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            verbose=cfg.LR_SCHEDULER_VERBOSE)
    ...
    def evaluate_model(self, loader):
        ...
        """
        self.model.eval()
        # collect as plain Python lists to avoid calling Tensor.numpy()
        # (some PyTorch builds may raise 'Numpy is not available' when .numpy() is used)
        y_true_list = []
        y_probs_list = []
        with torch.no_grad():
            for batch in loader:
                ...
                # get probabilities as Python floats
        ...
        # convert to numpy arrays for downstream metrics
        y_true = np.array(y_true_list)
        y_probs = np.array(y_probs_list)
        # sometimes labels all same -> roc_auc_score fails, handle gracefully
        try:
        ...
        return {"auc": auc, "acc": acc, "y_true": y_true, "y_probs": y_probs}
